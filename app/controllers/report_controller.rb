class ReportController < ApplicationController
  ...
  def apply
    ...
    @resultant = []
    give_sql_result(to_be_put_in_sql).each do |row|
  ...
  def give_sql_result(to_be_put_in_sql)
  ...
  join main_categories mc on (sc.main_cat_id = mc.id) where date_part('month',e.date) = 12
  and date_part('year',e.date) = 2014 #{to_be_put_in_sql} group by mc.category_name")
    res
  end
